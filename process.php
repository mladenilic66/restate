...
switch ($_REQUEST["form_id"]){
	...
	case "register":
		...
		$image = preg_replace("#[^a-z.0-9]#i","-",$image);
	    ...
	    if (!empty($image)) {
	        ...
	        if ($image_type != "png" && $image_type != "jpg") {
	            die(header("Location: /register.php?msg_r=Allowed image types: JPG,PNG"));
	        }

			if ($image_size > $image_maxsize) {
				die(header("Location: /register.php?msg_r=Max image size allowed: 1MB"));
		    } else {
		    	$save_image = move_uploaded_file($image_tmp,"/uploads/user_images/".$image);
		    }
	    } 

		$sql_reg = "INSERT into users(username,password,first_name,last_name,mobile_number,email,avatar,id_role,active) values('".$username."','".$hashed_password."','".$first_name."','".$last_name."','".$mobile_number."','".$email."','".$image."','2','1')";
	...
	case "add_real_estate":
		...
		$images = preg_replace("#[^a-z.0-9]#i","-",$images);
